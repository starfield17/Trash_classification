...
# 导入新增模型所需的库
try:
    from torchvision.models import resnet101, resnext50_32x4d, wide_resnet50_2, regnet_y_400mf, efficientnet_b4
    ADVANCED_MODELS_AVAILABLE = True
except ImportError:
    print("警告: 某些高级模型不可用，可能需要更新torchvision版本")
...
# 选择模型类型
# 轻量级: "resnet18_fpn", "mobilenet_v3"
# 标准版: "resnet50_fpn", "resnet50_fpn_v2"
# 高精度版: "resnet101_fpn", "resnext50_fpn", "wide_resnet50_fpn", "regnet_y_400mf_fpn", "efficientnet_b4_fpn"
MODEL_TYPE = "resnet50_fpn"  # 修改这里选择不同的模型
...
# 模型性能对比（在COCO数据集上的参考性能）
MODEL_PERFORMANCE = {
    "resnet18_fpn": {"mAP": 26.0, "params": "~20M", "速度": "快", "适用场景": "资源受限设备"},
    "mobilenet_v3": {"mAP": 32.8, "params": "~19M", "速度": "很快", "适用场景": "移动设备/树莓派"},
    "resnet50_fpn": {"mAP": 37.0, "params": "~42M", "速度": "中等", "适用场景": "标准应用"},
    "resnet50_fpn_v2": {"mAP": 38.4, "params": "~44M", "速度": "中等", "适用场景": "标准应用"},
    "resnet101_fpn": {"mAP": 39.8, "params": "~61M", "速度": "较慢", "适用场景": "高精度需求"},
    "resnext50_fpn": {"mAP": 40.2, "params": "~43M", "速度": "中等", "适用场景": "高精度需求"},  
    "wide_resnet50_fpn": {"mAP": 40.5, "params": "~69M", "速度": "较慢", "适用场景": "高精度需求"},
    "regnet_y_400mf_fpn": {"mAP": 37.8, "params": "~5M", "速度": "快", "适用场景": "高效高精度"},
    "efficientnet_b4_fpn": {"mAP": 41.3, "params": "~20M", "速度": "中等", "适用场景": "最佳精度/效率平衡"}
}
...
def print_model_recommendations():
    ...
    print("\n========== 模型选择建议 ==========")
    print("\n轻量级模型（适合资源受限环境）：")
    print("1. mobilenet_v3 - 超轻量(mAP~32.8)，适合树莓派")
    print("2. resnet18_fpn - 轻量(mAP~26.0)，速度快")
    
    print("\n标准模型（平衡性能）：")
    print("3. resnet50_fpn - 标准选择(mAP~37.0)")
    print("4. resnet50_fpn_v2 - 改进版(mAP~38.4)")
    
    print("\n高精度模型（相比ResNet50+FPN提升精度）：")
    print("5. regnet_y_400mf_fpn - 高效率高精度(mAP~37.8)，参数量极小")
    print("6. resnet101_fpn - 高精度(mAP~39.8)，经典选择")
    print("7. resnext50_fpn - 高精度(mAP~40.2)，训练速度适中")
    print("8. wide_resnet50_fpn - 很高精度(mAP~40.5)，需要更多显存")
    print("9. efficientnet_b4_fpn - 最高精度(mAP~41.3)，参数量适中")
    
    print("\n根据您的硬件选择：")
    print("- GPU显存 >= 8GB: efficientnet_b4_fpn 或 wide_resnet50_fpn")
    print("- GPU显存 4-8GB: resnext50_fpn 或 resnet101_fpn")  
    print("- GPU显存 < 4GB: regnet_y_400mf_fpn 或 resnet50_fpn_v2")
    print("- 部署到树莓派: mobilenet_v3 或 regnet_y_400mf_fpn")
    print("=====================================\n")
...
def get_faster_rcnn_model(num_classes, model_type="resnet50_fpn"):
    ...
    # ========== 新增高精度模型 ==========
    ...
    elif model_type == "resnet101_fpn" and ADVANCED_MODELS_AVAILABLE:
        # 高精度版：ResNet101+FPN (更深的网络，更高的精度)
        # 创建ResNet101 backbone
    ...
    elif model_type == "resnext50_fpn" and ADVANCED_MODELS_AVAILABLE:
        # 高精度版：ResNeXt50+FPN (更强的特征表示能力)
        # 创建ResNeXt50 backbone
    ...
    elif model_type == "wide_resnet50_fpn" and ADVANCED_MODELS_AVAILABLE:
        # 高精度版：Wide ResNet50+FPN (更宽的网络，更好的特征)
        # 创建Wide ResNet50 backbone
    ...
    elif model_type == "regnet_y_400mf_fpn" and ADVANCED_MODELS_AVAILABLE:
        # 高效高精度版：RegNetY-400MF+FPN (Meta的高效网络架构)
        # 创建RegNet backbone
        backbone = regnet_y_400mf(pretrained=True)
        
        # RegNet的特征提取
        features = backbone.trunk_output
        
        # 定义要返回的层
        return_layers = {
            'block1.block1_2': '0',  # 第一个stage的输出
            'block2.block2_3': '1',  # 第二个stage的输出
            'block3.block3_5': '2',  # 第三个stage的输出
            'block4.block4_1': '3'   # 第四个stage的输出
        }
        
        body = IntermediateLayerGetter(features, return_layers)
        
        # RegNetY-400MF的通道数
        in_channels_list = [48, 104, 208, 440]
    ...
    elif model_type == "efficientnet_b4_fpn" and ADVANCED_MODELS_AVAILABLE:
        # 高精度高效版：EfficientNet-B4+FPN
        # 创建EfficientNet backbone
        ...
        # EfficientNet-B4的通道数
        in_channels_list = [32, 56, 160, 448]
...
def adjust_hyperparameters(model_type, device):
    """根据模型类型和设备调整超参数"""
    # 默认参数
    params = {
        'batch_size': 8,
    ...
    # 根据模型类型调整
    if model_type in ["resnet101_fpn", "wide_resnet50_fpn"]:
        # 大模型需要更小的批次大小
        params['batch_size'] = 4 if device.type == "cuda" else 1
        params['learning_rate'] = 0.002  # 更低的学习率
    elif model_type in ["efficientnet_b4_fpn", "resnext50_fpn"]:
        params['batch_size'] = 6 if device.type == "cuda" else 2
        params['learning_rate'] = 0.003
    elif model_type == "regnet_y_400mf_fpn":
        # 高效模型可以使用更大批次
        params['batch_size'] = 12 if device.type == "cuda" else 4
    elif model_type in ["resnet18_fpn", "mobilenet_v3"]:
        # 轻量级模型
        params['batch_size'] = 16 if device.type == "cuda" else 4
        params['learning_rate'] = 0.01
    
    # CPU特殊处理
    if device.type == "cpu":
        params['num_workers'] = max(1, min(2, os.cpu_count() // 2))
...
def main():
    ...
    try:
        ...
        if MODEL_TYPE in MODEL_PERFORMANCE:
            ...
            print(f"预期性能 - mAP: {perf['mAP']}, 参数量: {perf['params']}, 速度: {perf['速度']}")
        ...
        # 根据模型类型和设备调整超参数
        hyperparams = adjust_hyperparameters(MODEL_TYPE, device)
        ...
        try:
            ...
            print(f"模型创建失败: {e}")
            if not ADVANCED_MODELS_AVAILABLE and MODEL_TYPE in ["resnet101_fpn", "resnext50_fpn", "wide_resnet50_fpn", "regnet_y_400mf_fpn", "efficientnet_b4_fpn"]:
                print("提示: 高级模型需要较新版本的torchvision，请更新后重试")
            return
