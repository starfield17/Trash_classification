...
class GarbageClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        # Extended classification layer
        self.model.classifier = nn.Sequential(
            nn.Linear(self.model.classifier[0].in_features, 1024),
            nn.Hardswish(),
            nn.Dropout(0.3),  # Increased dropout to prevent overfitting
            nn.Linear(1024, 512),
            nn.Hardswish(),
            nn.Dropout(0.2),
...
def train_model(model, train_loader, val_loader):
    ...
    while epoch < MAX_EPOCHS:
        ...
        if epochs_without_improvement >= EARLY_STOPPING_PATIENCE:
            print(f'\nNo improvement for {EARLY_STOPPING_PATIENCE} epochs, stopping training')
            break
