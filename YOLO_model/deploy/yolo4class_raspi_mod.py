...
class SerialManager:
    ...
    def queue_processor_thread(self):
        """Queue processing thread, periodically retrieves and sends data from queue"""
    ...
    def _process_queue_batch(self):
        ...
        try:
            # 4. Data preparation
            # Simplified: not resetting buffers, may cause additional delay
            # self.stm32_port.reset_input_buffer()
            # self.stm32_port.reset_output_buffer()
            # time.sleep(0.01)

            # 5. Assemble data packet, add header and tail identifiers
            data = bytes(
                [
            ...
            if DEBUG_WINDOW:
                ...
                print(
                    f"Data wait time in queue: {current_time - data_to_send['timestamp']:.3f} seconds"
                )
                print("-" * 50)
        except serial.SerialTimeoutException:
            print("Serial write timeout, device may not be responding")
            # Put timeout data back to queue to prevent data loss
            with self.queue_lock:
                self.send_queue.insert(0, data_to_send)
        except Exception as e:
            print(f"Serial send error: {str(e)}")
            # Consider putting failed data back to queue
            with self.queue_lock:
                # Only put back to queue if retry count doesn't exceed limit
    ...
    def send_to_stm32(self, class_id, center_x, center_y):
        ...
        with self.queue_lock:
            ...
            # Add data to queue, including original and scaled values
        ...
        if len(results) > 0:
            ...
            for box in boxes:
                ...
                if self._should_send_detection(
                    ...
                    # Directly call original send_to_stm32 method with required parameters
