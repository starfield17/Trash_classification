...
class QwenAutoLabeler:
    ...
    def detect_objects(self, base64_image):
        ...
        """
        prompt = (
            "请确认图像中是否存在以下11种垃圾物体中的任何一种，并准确识别其类别和位置：\n"
            "- 厨余垃圾类: potato（土豆）, daikon（白萝卜）, carrot（胡萝卜）\n"
            "- 可回收垃圾类: bottle（瓶子）(各种材质的), can（易拉罐）\n" 
            "- 有害垃圾类: battery（电池）, drug（药品）, inner_packing（内包装）\n"
            "- 其他垃圾类: tile（瓦片）, stone（石头）, brick（砖块）\n\n"
            "请注意，我只需要识别出属于这11种类别的物体，其他物体请忽略。\n"
            "对于每个检测到的物体，请提供以下信息：\n"
            "1. name: 必须是以上11种物体名称之一（使用英文名称，如potato, bottle等）\n"
            "2. 边界框坐标 (x1, y1, x2, y2)，表示左上角和右下角坐标\n"
            "3. 检测的置信度\n\n"
            "请以JSON格式返回结果，格式如下：\n"
            "```json\n"
            "{\n"
            ...
            "}\n"
            "```\n"
            "如果图像中没有任何属于上述11种类别的物体，请返回空的labels数组。\n"
            "请只返回JSON数据，不要添加任何其他说明或解释。"
        )
